...
Pages.showToothModal = async (patientId, toothNum) => {
  ...
  let history = [];
  try {
    history = await api.fetch(`/patients/${patientId}/tooth/${toothNum}/history`);
  } catch(_) {}
...
Pages.saveToothStatus = async (patientId, toothNum) => {
  ...
  try {
    await api.fetch(`/patients/${patientId}/dental-chart`, {
      method: 'PUT',
      body: JSON.stringify({ tooth_num: toothNum, status, notes }),
    });
    UI.closeModal();
...
Pages.saveAnamnesis = async (patientId) => {
  ...
  try {
    await api.fetch(`/patients/${patientId}/anamnesis`, { method:'PUT', body: JSON.stringify(body) });
...
Pages.completePlanItem = async (patientId, planId, itemId) => {
  try {
    await api.fetch(`/patients/${patientId}/treatment-plan/${planId}/item/${itemId}`, {
      method: 'PATCH',
...
Pages.showCreatePlanModal = async (patientId) => {
  ...
  document.getElementById('createPlanForm').addEventListener('submit', async (e) => {
    ...
    try {
      await api.fetch(`/patients/${patientId}/treatment-plan`, {
        method: 'POST',
        body: JSON.stringify({ title: formData.get('title'), notes: formData.get('notes'), items }),
      });
      UI.closeModal();
...
Pages.restorePatient = async (id) => {
  try {
    await api.fetch(`/patients/${id}/restore`, { method: 'POST' });
    UI.closeModal();
